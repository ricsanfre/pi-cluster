...
pipeline:
  ...
  outputs:
    - name: forward
      ...
      tls: true
      tls.verify: false
      # Fluent Bit >=5.0.4 defaults to true, which breaks Fluentd compatibility
      # See: https://docs.fluentbit.io/manual/installation/upgrade-notes#forward-output-retain_metadata_in_forward_mode-default-changed-to-true
      # See: https://github.com/fluent/fluent-bit/issues/11877
...
-   `retain_metadata_in_forward_mode`: Controls whether Fluent Bit embeds event metadata in the forward protocol payload. **Starting in Fluent Bit ≥5.0.4 this defaults to `true`**, which causes Fluentd to reject all events with `skip invalid event` warnings. Must be explicitly set to `false` when forwarding to a Fluentd aggregator to maintain compatibility.

    See [Fluent Bit upgrade notes](https://docs.fluentbit.io/manual/installation/upgrade-notes#forward-output-retain_metadata_in_forward_mode-default-changed-to-true) and [GitHub issue #11877](https://github.com/fluent/fluent-bit/issues/11877) for details.
