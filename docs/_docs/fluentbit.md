...
### Fluent-bit as log collector/forwarder

**Fluent-bit** will be deployed as logging agent instance,*forwarder* agent, collecting, parsing, filtering, enriching and forwarding logs to **Fluentd**, logs **aggregator**, which aggregates, post-process and route the logs to different destinations.
...
-   Step 2: Create Config Map containing fluent-bit configuration file
    ...
    {{site.data.alerts.note}}
    Previous config file is incomplete see section below to see a complete `fluent-bit.yaml` configuration to be applied to the cluster.
    {{site.data.alerts.end}}
...
The following data-pipelines will be configured to collect, transform and forward logs from the cluster
...
The environemnt variables that need to be set are related to Forward 
...
To enable it Fluent-bit HTTP server need to be enabled with the follwing configuration
