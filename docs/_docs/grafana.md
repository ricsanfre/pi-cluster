...
If you want to expose Grafana using a dedicated hostname such as `https://grafana.${CLUSTER_DOMAIN}/`, configure Grafana with that hostname as its public URL and make Envoy Gateway route requests to Grafana service without any path rewrite.

Add the following configuration to Helm `values.yaml` to expose Grafana through Envoy Gateway using Gateway API resources (i.e.: HTTPRoute and Gateway):
...
-   Replace `${CLUSTER_DOMAIN}` by  the domain name used in the cluster. For example: `homelab.ricsanfre.com`. `grafana.${CLUSTER_DOMAIN}` must resolve to Envoy Gateway's Load Balancer service external IP. External-DNS can be configured to publish those records automatically from `HTTPRoute` hostnames.
...
-   Step 2: Create a new OIDC client in 'picluster' Keycloak realm by navigating to:  Clients -> Create client
    ...
    -   Provide the following 'Logging settings'
      -   Valid redirect URIs: `https://grafana.${CLUSTER_DOMAIN}/login/generic_oauth`
      -   Home URL: `https://grafana.${CLUSTER_DOMAIN}/`
      -   Root URL: `https://grafana.${CLUSTER_DOMAIN}/`
      -   Web Origins: `https://grafana.${CLUSTER_DOMAIN}`
...
Create a secret containing environment variables `GF_AUTH_GENERIC_OAUTH_CLIENT_SECRET` and `GF_AUTH_GENERIC_OAUTH_CLIENT_ID` (OAuth2.0 client credentials)
...
data:
  GF_AUTH_GENERIC_OAUTH_CLIENT_SECRET: < grafana-client-secret | b64encode>
  GF_AUTH_GENERIC_OAUTH_CLIENT_ID: < grafana-client-id | b64encode>
```
...
-   `client_id` and `client_secret` are provided through environment variables loaded from a secret (`envFromSecret: grafana-env-secret`)
...
**About Prometheus Integration**

Expose Grafana with a dedicated hostname such as `https://grafana.${CLUSTER_DOMAIN}/` and keep `grafana.ini.server.root_url` aligned with that public URL.

This avoids redirecting in-cluster callers such as Prometheus to a rewritten public path and keeps Grafana's metrics endpoint reachable through the Kubernetes service.
