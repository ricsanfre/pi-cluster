...
description: How to configure a Raspberry Pi to host external services needed in HomeLab, like DNS authoritative server, PXE server, HA Proxy.
...
In order to ease the automation with Ansible, OS installed on **node1** is the same as the one installed in the nodes of the cluster: Ubuntu 24.04 64 bits.


## Storage Configuration

`node1` node is based on a Raspberry Pi 4B 4GB booting from a USB Flash Disk or SSD Disk.
Kingston A400 480GB SSD Disk and a USB3.0 to SATA adapter is connected to `node1`.
